...
int main() {
	...
	snprintf(driverPath, sizeof(driverPath), "%s\\R2S1.sys", currentPath); // 假设驱动程序文件名为 12356.sys
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// 停止服务的命令   R2S1这两个都是服务名
	const char* stopCmd = "sc stop R2S1";  // 修改为 MyDriver
	// 删除服务的命令
	const char* deleteCmd = "sc delete R2S1"; // 修改为 MyDriver
...
// 复制驱动文件到 C:\Windows\System32\drivers
	...
	// 创建服务命令     R2S1这两个都是服务名
	char createCmd[512];
	snprintf(createCmd, sizeof(createCmd), "sc create R2S1 binPath= \"C:\\Windows\\System32\\drivers\\R2S1.sys\" type= kernel start= demand", driverPath);
	...
	//	// 创建服务命令     R2S1这两个都是服务名
	//char createCmd[512];
	//snprintf(createCmd, sizeof(createCmd), "sc create R2S1 binPath= \"%s\" type= kernel start= demand", driverPath);
	//int resultCreate = system(createCmd);
	//if (resultCreate != 0) {
	//	printf("创建服务失败\n");
	//	return resultCreate;
	...
	// 启动服务命令
	const char* startCmd = "sc start R2S1";
	...
	result = RegOpenKeyEx(HKEY_LOCAL_MACHINE,
		TEXT("SYSTEM\\CurrentControlSet\\Services\\R2S1"),
		0,
		KEY_SET_VALUE,
		&hKey);
